...
const runResumeInsightsBackfill = async ({ apply = false, limit = 0, logger = console } = {}) => {
  ...
  for (const app of applications) {
    try {
      ...
      logger.error(`Backfill failed for application ${app._id}: ${error.message}`);
    }
  }
